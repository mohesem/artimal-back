...
async function createAnimalObject(animal, decoded) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (animal.entryType === 0 && !animal.pregnancyKey) reject({ status: 400, error: badRequest });

    // if (animal.entryType === 1 && !animal.price) reject({ status: 400, error: badRequest });
    if (animal.entryType === 1 && !animal.shopDate) reject({ status: 400, error: badRequest });
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (data.animal.entryType === 0) {
        const pregnancy = await trx.run(() => Pregnancies.document(data.animal.pregnancyKey));

        if (pregnancy.active) {
          const mother = await trx.run(() => Animals.document(data.animal.motherKey));
          await trx.run(() => Pregnancies.update(pregnancy, { active: false }));
          await trx.run(() => Animals.update(mother, { pregnant: false }));
        }

        console.log(pregnancy);

        console.log('sex: :: :: ', data.animal.sex);
